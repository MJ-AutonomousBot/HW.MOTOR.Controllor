...
//PID 제어 관련 변수 선언
float PID = 0 , P = 0 , I = 0, D = 0 ;
float erorr = 0, pre_error = 0, Lerorr = 0, Rerorr = 0;
...
void loop() {
  if (Serial.available() > 0) {                // 1. 필터 이용하여 전송하는 데이터에 대해서 오차 보정
    goal_vel = Serial.parseInt();              // 2. 듀티비 한계값 정의 및 조정
  ...
  if (Lerorr > 0.01) {
    Lerorr = goal_vel - Lcurvel ;
    P = Kp * Lerorr * (LAmicro - LpreMicro )  ;
    I = I + Ki * Lerorr  ;
    D = Kd * (Lerorr - pre_error) ;
    PID = P + I + D;
    pre_error = Lerorr;
    duty = setduty * PID;
    ...
    // 모터 입력 함수 부분
    if (duty > 50 && duty <= 200) {
      AMotor(duty, Forward); //A Cotrol true : Forward false : Backward
      BMotor(duty, Forward); //B Control
    }
    else if (duty < 0 && (abs(duty) > 50 && abs(duty) <= 200)) {
      AMotor(abs(duty), Backward);
      BMotor(abs(duty), Backward);
    }
    else if (duty == 0 ) {
      AMotor(duty, Forward);
      BMotor(duty, Forward);
